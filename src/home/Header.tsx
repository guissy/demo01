...
const Icon: React.FC<{ active: boolean }> = ({ active }) => {
  return <span className={active ? "transform rotate-180 transition-all inline-block" : "transition-all inline-block"}><DownOutlined /></span>
}
    ...
    <header>
      <section className="max-w-7xl mx-auto">
        <div className={cx("flex space-x-6 h-24 items-center", isSmall && 'justify-between')}>
          <Logo />
          {
            (isSmall && collapsed) && <Button type="link" onClick={() => setCollapsed((c) => !c)} style={{ marginBottom: 16 }}>
              {isSmall && <MenuOutlined />}
            </Button>
          }
          {(!isSmall || !collapsed) && <Menu mode={isSmall ? "vertical" : "horizontal"} className={cx(isSmall ? 'w-1/2' : 'flex-1')}
